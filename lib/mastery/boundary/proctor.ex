defmodule Mastery.Boundary.Proctor do
  ...
  defp start_quizzes(quizzes, now) do
    ...
    not_ready
    |> IO.inspect(label: "not_ready")
  end

  def start_quiz(quiz, now) do
    Logger.info("Starting quiz: #{inspect(quiz.fields.title)}")
  ...
  defp date_time_less_than_or_equal?(a, b), do: DateTime.compare(a, b) in [:lt, :eq]
end
